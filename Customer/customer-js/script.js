...
$(function(){
  ...
  $(function(){
    ...
    // custom faq-list active 실행
    $(function(){
      $('.faq-item .question').on('click', function(){
        const item = $(this).closest('.faq-item'); // 현재 클릭한 아이템
        const isOpen = item.hasClass('active'); // 이미 열려있는 상태인지 확인
    
        // 모든 항목 초기화
        $('.faq-item').removeClass('active');
        $('.faq-item .answer').slideUp(300);
    
        // 현재 항목만 열기 (열려있지 않았을 때만)
        if (!isOpen) {
      ...
      $(function(){
        ...
        $(function() {
          ...
          $('.menuwrap .text .menuBusiness.customers').on('click', function() {
